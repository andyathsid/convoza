...
type fakeStorageInspector struct {
	bucket   string
	metadata *firebaseClient.StorageObjectMetadata
	err      error
}
...
func (i *fakeStorageInspector) Inspect(
	_ context.Context,
	_ string,
) (*firebaseClient.StorageObjectMetadata, error) {
...
func validTestMetadata(objectPath string) *firebaseClient.StorageObjectMetadata {
	return &firebaseClient.StorageObjectMetadata{
		Path:           objectPath,
		ContentType:    "image/jpeg",
		Size:           1024,
		DownloadTokens: []string{"test-token"},
