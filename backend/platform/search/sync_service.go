...
// IndexMessageFromFirestoreChat resolves chat participants before indexing the
// message so access-scoped search can filter at query time instead of after pagination.
func (s *SyncService) IndexMessageFromFirestoreChat(ctx context.Context, fs *fsClient.FirestoreClient, doc MessageIndexDoc) error {
	participants, err := s.loadChatParticipants(ctx, fs, doc.ChatID)
	if err != nil {
		log.Printf("[search] IndexMessageFromFirestoreChat chatID=%s msgID=%s: %v", doc.ChatID, doc.ID, err)
		return err
	}
...
// IndexChatFromFirestore reads a chat and resolves searchable participant names
// from Firestore before indexing the chat and common-group documents.
// This is the shared helper used by both ChatService and MessageService.
func (s *SyncService) IndexChatFromFirestore(ctx context.Context, fs *fsClient.FirestoreClient, chatID string) {
	data, err := fs.GetChat(ctx, chatID)
	if err != nil {
		return
	}

	participantIDs := getStringSlice(data, "participants")
	profiles, err := fs.GetUserProfiles(ctx, participantIDs)
	if err != nil {
		log.Printf("[search] IndexChatFromFirestore profile resolution error chatID=%s: %v", chatID, err)
	}
	...
	chatDoc, groupDoc := buildChatIndexDocuments(chatID, data, usernames)
	_ = s.IndexChat(ctx, chatDoc)

	if groupDoc != nil {
		_ = s.IndexGroup(ctx, *groupDoc)
	}
}

func (s *SyncService) loadChatParticipants(ctx context.Context, fs *fsClient.FirestoreClient, chatID string) ([]string, error) {
	data, err := fs.GetChat(ctx, chatID)
	if err != nil {
		return nil, err
	}
	return getStringSlice(data, "participants"), nil
}

// ReindexUserProfile resolves the current Firestore profile before refreshing
// every query field that embeds the user's name.
func (s *SyncService) ReindexUserProfile(ctx context.Context, fs *fsClient.FirestoreClient, userID string) error {
	profiles, err := fs.GetUserProfiles(ctx, []string{userID})
	if err != nil {
		return err
	}
