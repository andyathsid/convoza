...
export type StorageUploadKind = ChatMediaType | "avatar" | "thumbnail";
...
const ALLOWED_CONTENT_TYPES: Record<StorageUploadKind, ReadonlySet<string>> = {
  ...
  audio: new Set(["audio/webm", "audio/mpeg", "audio/mp4", "audio/wav", "audio/x-wav", "audio/ogg", "audio/aac"]),
  document: new Set(["application/pdf", "application/msword", "application/vnd.openxmlformats-officedocument.wordprocessingml.document", "application/vnd.ms-excel", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "application/vnd.ms-powerpoint", "application/vnd.openxmlformats-officedocument.presentationml.presentation", "text/plain"]),
  thumbnail: new Set(["image/jpeg"]),
};

export function validateStorageUpload(kind: StorageUploadKind, data: Pick<Blob, "size" | "type">): string | null {
  ...
  if (!ALLOWED_CONTENT_TYPES[kind].has(data.type)) return `Unsupported ${kind} file type`;
  return null;
}
