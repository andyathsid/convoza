...
import { db, markMessagesAsRead, markMessagesDelivered } from "@/lib/firebase";
import {
...
import type { InChatMessageFilter } from "@/stores/ui-store";
  ...
  const markReceipts = useCallback(
    (messages: FirestoreMessage[]) => {
      ...
      if (deliveredIds.length > 0) {
        void markMessagesDelivered(effectiveFilter.chatId, deliveredIds, currentUserId);
      }
      if (readIds.length > 0) {
        void markMessagesAsRead(effectiveFilter.chatId, readIds, currentUserId);
      }
    },
