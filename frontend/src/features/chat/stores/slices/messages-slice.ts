...
function getTimestampMillis(value: FirestoreMessage["createdAt"] | undefined): number {
  return normalizeChatDate(value)?.getTime() ?? Number.NaN;
}
...
> = (set, get) => ({
  ...
  markChatAsRead: (chatId: string) => {
    ...
    void markMessagesAsRead(chatId, ids, readThroughMessageId).then((didMarkRead) => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
        ...
        if (wasJumpJustCompleted) {
          ...
          if (existingIdSet.has(newestSnapId)) {
            ...
            const oldestNewTimestamp = latestMessages.length > 0
              ? getTimestampMillis(latestMessages[0].createdAt)
              : Infinity;

            const newestNewTimestamp = latestMessages.length > 0
              ? getTimestampMillis(latestMessages[latestMessages.length - 1].createdAt)
              : 0;

            const newestExistingTimestamp = existingMessages.length > 0
              ? existingMessages.reduce((max, m) => {
                  const mTime = getTimestampMillis(m.createdAt);
      ...
      const reconciled = isFromCache ? pending : pending.filter((pm) => {
        ...
        return !merged.some((m) => {
          ...
          const mTime = getTimestampMillis(m.createdAt);
          const pmTime = getTimestampMillis(pm.createdAt);
      ...
      if (wasJumpJustCompleted) {
        ...
        const mergedNewestTime = getTimestampMillis(newestMsg?.createdAt);
      ...
      if (currentUserId && shouldUpdateMessages && !mergeSkipped) {
        ...
        if (undelivered.length > 0) {
          markMessagesDelivered(chatId, undelivered.map((m) => m.id!).filter(Boolean));
        }
