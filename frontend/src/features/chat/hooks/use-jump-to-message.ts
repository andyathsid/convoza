...
import type { VirtualItem } from "../types/virtual-item";
...
interface UseJumpToMessageReturn {
  ...
  jumpScrollJustCompletedRef: React.RefObject<boolean>;
  clearHighlight: () => void;
...
export function useJumpToMessage({
  ...
  const jumpScrollJustCompletedRef = useRef(false);
  ...
  }, [virtualizerRef, allItemsRef, shouldAutoScrollRef, hasMoreMessages, onLoadMore, chatId, storeJumpToMessage, findTargetIndex, getStoreState]);
