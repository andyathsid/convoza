...
import { formatMessageTime, normalizeChatDate } from "@/features/chat/lib/helper";
...
function monthLabel(message: FirestoreMessage): string {
  const date = normalizeChatDate(message.createdAt);
  return date ? format(date, "MMMM yyyy") : "Unknown date";
}

function groupByMonth(items: FirestoreMessage[]) {
...
export function ChatAttachmentsPreview({ messages, onOpenGallery, className }: PreviewProps) {
  const [lightbox, setLightbox] = useState<{ items: FirestoreMessage[]; index: number } | null>(null);
  ...
  const loadMore = useCallback(async (tab: AttachmentTab) => {
    ...
    try {
      ...
      const nextItems = snapshot.docs
        .map((messageDoc) => ({ id: messageDoc.id, ...messageDoc.data() } as FirestoreMessage))
...
function senderLabel(item: FirestoreMessage, currentUserId: string | null): string {
  return item.senderId === currentUserId ? "You" : item.senderName || "Unknown";
}

function DocumentList({ items, currentUserId }: { items: FirestoreMessage[]; currentUserId: string | null }) {
  return (
...
function AudioList({ items, currentUserId }: { items: FirestoreMessage[]; currentUserId: string | null }) {
  return (
