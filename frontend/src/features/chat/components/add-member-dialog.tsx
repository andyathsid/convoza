...
export default function AddMemberDialog({
  ...
  const existingMemberIdsRef = useRef(existingMemberIds);
	...
	const { users: directoryUsers, hasMore, isLoading, reset: resetDirectory, loadNextPage, search: searchDirectory } = useUserDirectory();
  ...
  useEffect(() => {
    if (!open) return;
		resetDirectory();
		void loadNextPage("");
	}, [open, loadNextPage, resetDirectory]);

  const displayUsers = useCallback(() => {
	const users = searchQuery.trim() ? searchResults : Array.from(directoryUsers.values());
	if (!searchQuery.trim()) return users.filter((u) => !existingMemberIds.includes(u.id));
    const q = searchQuery.toLowerCase();
    return users.filter(
      (u) =>
		u.username.toLowerCase().includes(q)
    );
	}, [directoryUsers, existingMemberIds, searchQuery, searchResults]);
  ...
  const handleSearch = useCallback(
    (query: string) => {
      ...
      searchTimeoutRef.current = setTimeout(async () => {
		try {
			setSearchResults((await searchDirectory(query, "")).filter((u) => !existingMemberIds.includes(u.id)));
		} catch (error) {
	...
	[searchDirectory, existingMemberIds]
  );
          ...
          >
            {selectedIds.length > 0 && (
              <div className="px-4 py-2 flex flex-wrap gap-2 border-b border-border">
                {selectedIds.map((uid) => {
				  const profile = useProfiles.getState().getProfile(uid) || Array.from(directoryUsers.values()).find((u) => u.id === uid) || searchResults.find((u) => u.id === uid);
                  return (
                    <button
                      key={uid}
                      type="button"
            ...
            {isLoading ? (
              ...
              <div className="px-4 py-2">
				...
				{hasMore && !isLoading && !searchQuery.trim() && (
				  <Button variant="ghost" className="w-full" onClick={() => void loadNextPage("")}>Load more contacts</Button>
				)}
              </div>
            )}
          </ScrollArea>
