...
import ChatBody from "@/features/chat/components/chat-body";
import ChatFooter from "@/features/chat/components/chat-footer";
import EmptyState from "@/components/empty-state";
import MediaSkeleton from "@/features/chat/components/media-skeleton";
import InfoPanel from "@/features/chat/components/chat-info-panel";
import InChatSearchPanel from "@/features/chat/components/in-chat-search-panel";
import { Spinner } from "@/components/ui/spinner";
import { cn } from "@/lib/utils";
import { useFilteredChatMessages } from "@/features/chat/hooks/use-filtered-chat-messages";
...
export default function ChatIdPage({ params }: { params: Promise<{ chatId: string }> }) {
  ...
  const ensureFilteredMessage = filteredMessages.ensureMessage;
  const isFilteredMessagesInitialLoading = filteredMessages.isInitialLoading;
  const filteredMessageCount = filteredMessages.messages.length;
  ...
  useEffect(() => {
    if (
      !activeMessageFilter ||
      !searchJumpTargetId ||
      isFilteredMessagesInitialLoading ||
    ...
    let cancelled = false;
    void ensureFilteredMessage(searchJumpTargetId).then((found) => {
