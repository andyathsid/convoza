...
export default function NewChatPanel() {
  ...
  const { searchUsers, createChat, updateGroupAvatar } = useChatStore();
  ...
  // Fetch all known contacts from backend
  const [allUsers, setAllUsers] = useState<Map<string, User>>(new Map());
  const [isLoadingUsers, setIsLoadingUsers] = useState(false);
  const { getAllUsers } = useChatStore();

  useEffect(() => {
    if (!currentUserId) return;
    startTransition(() => setIsLoadingUsers(true));
    getAllUsers().then((users) => {
      ...
      if (users.length > 0) {
        useProfiles.getState().ensureProfiles(users.map((u) => u.id));
      }
    });
                ...
                Array.from(allUsers.entries()).map(([uid, user]) => {
                  ...
                  return (
                    <button
                      ...
                      type="button"
                      className="w-full flex items-center gap-3 px-4 py-3 hover:bg-accent/50 transition-colors disabled:opacity-50 disabled:pointer-events-none"
                      onClick={() => handleCreateDirectChat({ id: uid, username: displayName, avatar: displayAvatar, email: user.email } as User)}
                      disabled={isCreating}
                    >
                  ...
                  filtered.map(([uid, user]) => {
                    ...
                    return (
                      <button
                        ...
                        onClick={() => toggleMember({ id: uid, username: displayName, avatar: displayAvatar, email: user.email } as User)}
                      >
                        <div className="relative">
                          <AvatarWithBadge name={displayName} src={displayAvatar} />
                          {isSelected && (
